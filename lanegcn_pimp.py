...
class StateEstimator(nn.Module):
    def __init__(self, config):
        ...
        self.rnn = nn.RNN(input_size=config["rnn/input_dims"], hidden_size=config["rnn/hidden_dims"], num_layers=config["rnn/num_layers"], batch_first=True, bidirectional=True)
        self.decoder = nn.Linear(config["rnn/hidden_dims"]*2*config["rnn/num_layers"], 2)
    
    def forward(self, features):
        """
        features is a tensor of shape (N,L,3)
        """
        _, h_n = self.rnn(features)
        h_n = torch.flatten(h_n.permute(1,0,2), start_dim=1)
...
class BoundedResidual(nn.Module):
    ...
    def forward(self, pred_states, encoded_vector):
        augmented_state = torch.hstack([pred_states, encoded_vector])
        pred_residuals = self.resid(augmented_state)
        pred_residuals *= self.epsilon
        pred_residuals = pred_residuals.reshape((-1, config['num_preds'], 2))
...
class PhysicsHeader(nn.Module):
    def __init__(self, config):
        ...
        self.brm = BoundedResidual(config)
    ...
    def _assemble_states(self, features):
        ...
        estimated_states = self.state_estimator(features)
        positions = features[:,-1,:2]
        states = torch.hstack([positions,estimated_states])
        return gpu(states)
    
    def _activation_function(self, raw_control_inputs):
        """Raw control inputs in the shape [ACTORS, PREDS, 2]"""
        steer = raw_control_inputs[...,0]
        speed = raw_control_inputs[...,1]
        steer = np.pi*torch.tanh(steer)
    ...
    def forward(self, actors, features):
        ...
        control_inputs = self._activation_function(control_inputs)
        integrated_states = self.dynamics(initial_states, control_inputs)
        output_states = integrated_states[:,:,:2]
        resid_input = torch.flatten(integrated_states.clone().detach(), start_dim=1)
        residuals = self.brm(resid_input, actors.clone().detach())
...
class PredLoss(nn.Module):
    ...
    def forward(self, out: Dict[str, List[Tensor]], gt_preds: List[Tensor], has_preds: List[Tensor], curriculum_preds) -> Dict[str, Union[Tensor, int]]:
        ...
        # -- We implement the curriculum by modifying the num_preds here -- #
        num_preds = min(num_preds, curriculum_preds)
...
class Loss(nn.Module):
    ...
    def forward(self, out: Dict, data: Dict, curriculum_preds: int) -> Dict:
        loss_out = self.pred_loss(out, gpu(data["gt_preds"]), gpu(data["has_preds"]), curriculum_preds)

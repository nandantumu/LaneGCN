...
RUN apt install -y python3 python3-pip git wget curl libgl1-mesa-glx apt-utils libglib2.0-0 cmake openmpi-bin openmpi-doc libopenmpi-dev
...
RUN pip3 install jupyterlab_nvdashboard mpi4py scikit-image scikit-learn ipdb tqdm gpustat
RUN pip3 install --no-cache-dir horovod[pytorch]
